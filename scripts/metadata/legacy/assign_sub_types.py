"""Legacy one-shot tool for assigning sub_type to archived behavior metadata.

The active behavior_gen schema does not use sub_type. This script is retained
only for reproducing the legacy metadata format and must not target production.
...
SRC = (
    ...
    / "archive"
    / "behavior"
    / "real_behavior_metadata_legacy_compatible.json"
)
